  ...
  void initState() {
    ...
    if (widget.offerModel == null) {
      ...
      _selected = List.generate(dayNameList.length,
          (i) => requestModel.recurringDays.contains(i) ? true : false);
      _selectOnAfter();
      endType = requestModel.end.endType == "on" ? 0 : 1;
      selectedDate = requestModel.end.endType == "on"
          ? DateTime.fromMillisecondsSinceEpoch(requestModel.end.on)
          : DateTime.now();
      after = requestModel.end.endType == "on"
          ? "1"
          : requestModel.end.after.toString();
    } else {
      offerModel = widget.offerModel;
      isRecurring =
          widget.offerModel.isRecurring || widget.offerModel.autoGenerated;
      _selected = List.generate(dayNameList.length,
          (i) => offerModel.recurringDays.contains(i) ? true : false);
      _selectOnAfter();
