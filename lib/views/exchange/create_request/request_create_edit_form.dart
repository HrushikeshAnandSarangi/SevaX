...
class RequestCreateEditFormState extends State<RequestCreateEditForm> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    widget.formType == RequestFormType.CREATE ? _initializeCreateRequestModel() : _initializeEditRequestModel();
  ...
  _initializeCreateRequestModel() {
    ...
    if (widget.isOfferRequest ?? false) {
      ...
      requestModel.goodsDonationDetails.requiredGoods = widget.offer.goodsDonationDetails.requiredGoods;
    }
  ...
  void didChangeDependencies() {
    ...
    if (widget.loggedInUser?.sevaUserID != null)
      FirestoreManager.getUserForIdStream(sevaUserId: widget.loggedInUser.sevaUserID).listen((userModel) {});
                                ...
                                switch (requestModel.requestType) {
                                  ...
                                  case RequestType.ONE_TO_MANY_REQUEST:
                                    return TimeRequest(
                                      ...
                                      projectModelList: projectModelList,
                                      selectedInstructorModel: selectedInstructorModel,
                                      selectedInstructorModelChanged: (instructorModel, isSelected) {
                              ...
                              child: Center(
                                child: Container(
                                  child: CustomElevatedButton(
                                    onPressed: widget.formType == RequestFormType.EDIT ? editRequest : createRequest,
                                    child: Text(
  ...
  Widget RequestTypeWidgetCommunityRequests() {
    return (widget.formType == RequestFormType.CREATE && requestModel.requestMode == RequestMode.TIMEBANK_REQUEST)
        ? Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(
                    ...
                    child: ConfigurationCheck(
                      ...
                      child: requestUtils.optionRadioButton<RequestType>(
                        ...
                        onChanged: (value) {
                          ...
                          AppConfig.helpIconContextMember = HelpContextMemberType.one_to_many_requests;
                          setState(() => {});
                        },
                      ),
                    ),
  ...
  Widget RequestTypeWidgetPersonalRequests() {
    return (widget.formType == RequestFormType.CREATE && requestModel.requestMode == RequestMode.PERSONAL_REQUEST)
        ? Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(
          ...
          children: {
            0: Text(
              timebankModel.parentTimebankId == FlavorConfig.values.timebankId
                  ...
                  : S.of(context).seva + " ${timebankModel.name}" + " ${S.of(context).group} " + S.of(context).request,
  ...
  void createRequest() async {
    ...
    DateTime startDate = DateTime.fromMillisecondsSinceEpoch(OfferDurationWidgetState.starttimestamp);
    ...
    if (requestModel.isRecurring) {
      ...
      requestModel.end.on =
          requestModel.end.endType == S.of(context).on ? RepeatWidgetState.selectedDate.millisecondsSinceEpoch : null;
      requestModel.end.after =
          (requestModel.end.endType == S.of(context).after ? int.parse(RepeatWidgetState.after) : null);
    }
    ...
    if (_formKey.currentState.validate()) {
      ...
      if (requestModel.requestStart == 0 || requestModel.requestEnd == 0) {
        ...
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).validation_error_no_date, context: context);
        return;
      }
      ...
      if (OfferDurationWidgetState.starttimestamp > OfferDurationWidgetState.endtimestamp) {
        Scrollable.ensureVisible(_dateKey.currentContext);
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).validation_error_end_date_greater, context: context);
        return;
      }

      if (DateTime.fromMillisecondsSinceEpoch(OfferDurationWidgetState.starttimestamp).isBefore(DateTime.now())) {
        Scrollable.ensureVisible(_dateKey.currentContext);
        requestUtils.showDialogForTitle(context: context, dialogTitle: S.of(context).past_time_selected);
        return;
      }
      ...
      if (requestModel.requestType == RequestType.GOODS &&
        ...
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).goods_validation, context: context);
        return;
      }
      if (requestModel.requestType == RequestType.BORROW &&
          requestModel.roomOrTool == LendingType.ITEM.readable //because was throwing dialog when creating for place
          &&
          (requestModel.borrowModel.requiredItems == null || requestModel.borrowModel.requiredItems.isEmpty)) {
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).items_validation, context: context);
        return;
      }
      if (requestModel.requestType == RequestType.BORROW && requestModel.location == null) {
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).validation_error_location, context: context);
        return;
      }
...
//Review done or not to be used to find out if Borrow request is completed or not
      ...
      if (requestModel.requestType == RequestType.ONE_TO_MANY_REQUEST &&
        ...
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).select_a_speaker_dialog, context: context);
        return;
      }
        ...
        case RequestMode.PERSONAL_REQUEST:
          ...
          if (!onBalanceCheckResult.hasSuffiientCredits) {
            ...
            await sendInsufficentNotificationToAdmin(
                creditsNeeded: onBalanceCheckResult.credits, context: context, timebankModel: timebankModel);
            return;
          }
          break;
      ...
      if (selectedInstructorModel != null && requestModel.requestType == RequestType.ONE_TO_MANY_REQUEST) {
      ...
      await requestUtils.createProjectOneToManyRequest(
          context: context, requestModel: requestModel, projectModel: widget.projectModel, createEvent: createEvent);
  ...
  void continueCreateRequest({BuildContext confirmationDialogContext}) async {
    ...
    List<String> resVar = await writeToDB(
        context: context, timebankModel: timebankModel, requestModel: requestModel, offer: widget.offer);
    eventsIdsArr = resVar;
    await _updateProjectModel();
    // Navigator.of(context, rootNavigator: true).pop();
    if (dialogContext != null) Navigator.pop(dialogContext);
  ...
  void editRequest() async {
    ...
    if (_formKey.currentState.validate()) {
      ...
      if (OfferDurationWidgetState.starttimestamp == 0 || OfferDurationWidgetState.endtimestamp == 0) {
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).validation_error_no_date, context: context);
        return;
      }
      ...
      logger.i("${OfferDurationWidgetState.starttimestamp} |||||| ${DateTime.now().millisecondsSinceEpoch}");
      ...
      if (OfferDurationWidgetState.starttimestamp < DateTime.now().millisecondsSinceEpoch ||
        ...
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).past_time_selected, context: context);
        return;
      }
      ...
      if (requestModel.requestType == RequestType.ONE_TO_MANY_REQUEST &&
          (selectedInstructorModel == {} || selectedInstructorModel == null || instructorAdded == false)) {
        requestUtils.showDialogForTitle(dialogTitle: S.of(context).select_a_speaker, context: context);
        return;
      }

      //Calculate session duration of one to many request using request start and request end time
      if (requestModel.requestType == RequestType.ONE_TO_MANY_REQUEST) {
        if (OfferDurationWidgetState.starttimestamp != null && OfferDurationWidgetState.endtimestamp != null) {
          DateTime startDateNew = DateTime.fromMillisecondsSinceEpoch(OfferDurationWidgetState.starttimestamp);
          DateTime endDateNew = DateTime.fromMillisecondsSinceEpoch(OfferDurationWidgetState.endtimestamp);
...
*/
  ...
  int calculateRecurrencesOnMode(RequestModel requestModel) {
    ...
    while (lastRound == false) {
      eventStartDate = DateTime(eventStartDate.year, eventStartDate.month, eventStartDate.day + 1, eventStartDate.hour,
          eventStartDate.minute, eventStartDate.second);
